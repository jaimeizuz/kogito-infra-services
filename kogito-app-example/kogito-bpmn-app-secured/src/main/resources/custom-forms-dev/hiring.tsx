...
const Form__hiring: React.FC<any> = (props: any) => {
	...
	const [candidateData__skills, set__candidateData__skills] = useState<
		string[]
	>([]);
	const [candidateDataStorageResult, set__candidateDataStorageResult] =
		useState<string>('');
	const [errorStrategy, set__errorStrategy] = useState<string>('');
	const [expirationTime, set__expirationTime] = useState<string>('');
	const [needMgmtApproval, set__needMgmtApproval] = useState<boolean>(false);
	const [throwException, set__throwException] = useState<boolean>(false);
	const [workitemType, set__workitemType] = useState<string>('');
	...
	const setFormData = (data) => {
		...
		set__candidateDataStorageResult(data?.candidateDataStorageResult ?? '');
		set__errorStrategy(data?.errorStrategy ?? '');
		set__expirationTime(data?.expirationTime ?? '');
		set__needMgmtApproval(data?.needMgmtApproval ?? false);
		set__throwException(data?.throwException ?? false);
		set__workitemType(data?.workitemType ?? '');
	};
	...
	const getFormData = useCallback(() => {
		...
		formData.candidateDataStorageResult = candidateDataStorageResult;
		formData.errorStrategy = errorStrategy;
		formData.expirationTime = expirationTime;
		formData.needMgmtApproval = needMgmtApproval;
		formData.throwException = throwException;
		formData.workitemType = workitemType;
		return formData;
	}, [
		...
		<div className={'pf-c-form'}>
			<Card>
				<CardBody className='pf-c-form'>
					...
					<FormGroup
						fieldId={'uniforms-000k-0002'}
						label={'Email'}
						isRequired={false}>
						<TextInput
							name={'candidateData.email'}
							id={'uniforms-000k-0002'}
							isDisabled={false}
							placeholder={''}
							type={'text'}
					...
					<FormGroup
						fieldId={'uniforms-000k-0005'}
						label={'Last name'}
						isRequired={false}>
						<TextInput
							name={'candidateData.lastName'}
							id={'uniforms-000k-0005'}
							isDisabled={false}
							placeholder={''}
							type={'text'}
					...
					<FormGroup
						fieldId={'uniforms-000k-0006'}
						label={'Name'}
						isRequired={false}>
						<TextInput
							name={'candidateData.name'}
							id={'uniforms-000k-0006'}
							isDisabled={false}
							placeholder={''}
							type={'text'}
							...
							{candidateData__skills?.map((_, itemIndex) => (
								<div
									...
									<div style={{ width: '100%', marginRight: '10px' }}>
										<FormGroup
											fieldId={'uniforms-000k-0009'}
											label={''}
											isRequired={false}>
											<TextInput
												name={`candidateData__skills.${itemIndex}`}
												id={'uniforms-000k-0009'}
												isDisabled={false}
												placeholder={''}
												type={'text'}
			...
			<FormGroup
				fieldId={'uniforms-000k-000a'}
				label={'Candidate data storage result'}
				isRequired={false}>
				<TextInput
					name={'candidateDataStorageResult'}
					id={'uniforms-000k-000a'}
					isDisabled={false}
					placeholder={''}
					type={'text'}
					value={candidateDataStorageResult}
					onChange={set__candidateDataStorageResult}
				/>
			</FormGroup>
			<FormGroup
				fieldId={'uniforms-000k-000b'}
				label={'Error strategy'}
				isRequired={false}>
				<TextInput
					name={'errorStrategy'}
					id={'uniforms-000k-000b'}
					isDisabled={false}
					placeholder={''}
					type={'text'}
					value={errorStrategy}
			...
			<FormGroup
				fieldId={'uniforms-000k-000c'}
				label={'Expiration time'}
				isRequired={false}>
				<TextInput
					name={'expirationTime'}
					id={'uniforms-000k-000c'}
					isDisabled={false}
					placeholder={''}
					type={'text'}
					value={expirationTime}
					onChange={set__expirationTime}
				/>
			</FormGroup>
			<FormGroup fieldId='uniforms-000k-000e'>
				<Checkbox
					isChecked={needMgmtApproval}
					isDisabled={false}
					id={'uniforms-000k-000e'}
					name={'needMgmtApproval'}
					label={'Need mgmt approval'}
					onChange={set__needMgmtApproval}
				/>
			</FormGroup>
			<FormGroup fieldId='uniforms-000k-000g'>
				<Checkbox
					isChecked={throwException}
					isDisabled={false}
					id={'uniforms-000k-000g'}
					name={'throwException'}
			...
			<FormGroup
				fieldId={'uniforms-000k-000h'}
				label={'Workitem type'}
				isRequired={false}>
				<TextInput
					name={'workitemType'}
					id={'uniforms-000k-000h'}
					isDisabled={false}
					placeholder={''}
					type={'text'}
					value={workitemType}
					onChange={set__workitemType}
				/>
			</FormGroup>
		</div>
	);
